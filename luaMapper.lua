local mainFile = [=[-- This file was generated using Luraph Obfuscator v13.7
...
local TESTLOCAL = game.ReplicatedStorage
local v15 = game:GetService('RunService')
local v14 = game:GetService('Players')
local v25 = game:GetService('StarterGui')
local v26 = game:GetService('TweenService')
local v15 = game:GetService('RunService')
local v19 = game:GetService('ReplicatedStorage').Parent.ReplicatedStorage
...
do
	function strings.normalize(str)
		return str:sub(0, 1):lower() .. str:sub(2, #str)
	end
	function strings.capitalize(str)
		return str:sub(0, 1):upper() .. str:sub(2, #str)
	end
end
...
do
	...
	mapper.inferMap = {
		{
			name = 'getServiceLocals',
			check = function(line)
				local varName, serviceString, indexerFound = line:match('(%w+)%s*=%s*game%:GetService%(%W?(%a+)%W?%)([%.%[%:]?)')
				if varName and #serviceString > 0 then
					if #indexerFound > 0 then
						-- print(`inferMap::getServiceLocals::indexerFound {varName} {serviceString}`)
						return varName, nil
					end
					return varName, (serviceString:find('ervice') and strings.normalize(serviceString):gsub('service', 'Service')) or (strings.normalize(serviceString)..'Service')
				end
				return varName, nil
			end
		},
		{
			name = 'getInstanceLocals',
			check = function(line)
				local varName, varValue = line:match('local (%w+)%s*=%s*([%w%.%[%]%"%\'%s]*)')
				if varName and #varValue > 0 then
					local prefix = 'l'
					if varValue == 'Instance.new' then -- Instance.new mutation
						varValue = line:match('Instance%.new%(%W(%a+)%W,?%s*[%a]*%)')
						varValue ..= 'Instance'
						prefix = 'i'
					elseif varValue == 'true' or varValue == 'false' then -- boolean mutation
						prefix = 'b'
					elseif tonumber(varValue) then -- number mutation
						prefix = 'n'
					elseif varValue:sub(0, 4) == 'game' and varValue:find('%.') and not (varValue:sub(6, #varValue)):find('%.') then -- service mutation
						local serviceString = varValue:sub(6, #varValue)
						print(`inferMap::getInstanceLocals::serviceMutation {varName} -> {varValue}`)
 						return varName, (serviceString:find('ervice') and strings.normalize(serviceString):gsub('service', 'Service')) or (strings.normalize(serviceString)..'Service')
					end
					--print(`inferMap::setLocals {varName} -> {varValue}`)
					local replaceVar = strings.normalize(varValue:gsub('[%.%s]+', '_'):gsub('%[[%"%\']+', '_'):gsub('[%"%\']%]', ''))
					if not mapper.varNames[replaceVar] then
						mapper.varNames[replaceVar] = -1
					end
					mapper.varNames[replaceVar] += 1
					return varName, `{prefix}_{replaceVar}{mapper.varNames[replaceVar] > 0 and '_'..mapper.varNames[replaceVar] or ''}`
				end
				return varName, nil
			end
		}
	...
	function mapper.inferVar(line)
		for _, module in next, mapper.inferMap do
			local original, guess = module.check(line)
			if guess then
				print(`inferVar::{module.name} {original} >> {guess}`)
	...
	function mapper.remapVar(var, new)
		for id, line in next, mapper.mappings do
			local patch = line:gsub(`{var}(%W)`, function(match)
				return new .. match
			end)
			if line ~= patch then
				print(`remapVar::{id} {var} -> {new}`)
			end
...
mapper.setSource(mainFile)

local varsToRemap = {}
for _, line in next, mapper.mappings do
	local varName, varDetected = mapper.inferVar(line)
	if varDetected then
		table.insert(varsToRemap, { name = varName, replace = varDetected })
	end
end
--[[
for _, varMap in next, varsToRemap do
	mapper.remapVar(varMap.name, varMap.replace)
end

print(mapper.compile())
]]
